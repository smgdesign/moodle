 ...
 */
function course_get_cm_rename_action(cm_info $mod) {
 ...
 */
function mod_duplicate_activity($course, $cm) {
    ...
    if ($newcm) {
        ...
        $resp->fullcontent = $courserenderer->course_section_cm_list_item($course, $completioninfo, $newcm, null);
        $resp->cmid = $newcm->id;
    } else {
        // Trigger a redirect
        $resp->redirect = true;
