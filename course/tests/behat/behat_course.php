...
class behat_course extends behat_base {
    ...
    public function i_delete_activity($activityname) {
        ...
        if ($this->running_javascript()) {
            ...
            $actionselement = $this->get_activity_element(get_string('actions', 'moodle'), 'link', $activityname);
    ...
    public function i_duplicate_activity($activityname) {
        ...
        $steps = array_merge($steps, array(
            new Given('I click on "' . get_string('duplicate') . '" "link" in the "' . $activity . '" activity'),
