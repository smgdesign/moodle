...
var COMPONENTNAME = 'atto_link',
    CSS = {
        ...
        URLMISSINGHTTPWARNING: 'atto_link_missinghttpwarning'
    },
    SELECTORS = {
        URLINPUT: '.atto_link_urlentry'
    },
    TEMPLATE = '' +
            '<form class="atto_form">' +
                ...
                '<div style="display:none" role="alert" class="warning {{CSS.URLMISSINGHTTPWARNING}}">' +
                    '{{get_string "missinghttp" component}}' +
                '</div>' +
...
Y.namespace('M.atto_link').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _getDialogueContent: function() {
        ...
        this._content.one('.' + CSS.URLINPUT).on('change', this._URLProtocolSpecified, this);
    ...
    /**
     * Ensures that the provided URL specifies a protocol.
     *
     * It implies that no moodle-relative URLs are accepted.
     *
     * @method _URLProtocolSpecified
     * @return {boolean} whether a warning should be displayed
     * @private
     */
    _URLProtocolSpecified : function() {

        var url = this._content.one('.url').get('value');

        if (new RegExp("[a-z]+:\/\/").test(url) === false) {
            this._content.one('.' + CSS.URLMISSINGHTTPWARNING).setStyle('display', 'block');
            this._content.one('.' + CSS.URLINPUT).setAttribute('aria-invalid', true);
            return false;
        } else {
            this._content.one('.' + CSS.URLMISSINGHTTPWARNING).setStyle('display', 'none');
            this._content.one('.' + CSS.URLINPUT).setAttribute('aria-invalid', false);
            return true;
        }
    }
});
